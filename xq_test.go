...
func TestSelect(t *testing.T) {
	x := New().Sel("shop").Cols("name", "addr", "city").Where(IfRule{R: R{"id = ?": 1}})
	t.Log(x.Str())
}
...
func TestJoin(t *testing.T) {
	...
	x := New().Sel("mshop m").Cols("m.id", "m.name", "m.addr", "m.city").Join("shop s",
		...
		Where(IfRule{R: R{"m.closed = ?": false}}).Where(IfRule{Ignore: len(city) == 0, R: R{"m.city = ?": city}}).
...
func TestWhereOr(t *testing.T) {
	x := New().Sel(`shop`).Cols(`id`, `name`).WhereOr(IfRule{Ignore: true, R: R{`last_raw_dt is null`: nil}}, IfRule{Ignore: false, R: R{`last_raw_dt < last_search_dt`: nil}}).
		Where(IfRule{R: R{`last_search_dt is not null`: nil}}).Order(`last_search_dt asc`).Limit(0, 1)
	t.Log(x.Str())
}
